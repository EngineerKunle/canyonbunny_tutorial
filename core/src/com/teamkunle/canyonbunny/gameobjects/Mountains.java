...
public class Mountains extends AbstractGameObject{
	...
	public void render(SpriteBatch sb) {
		...
		drawMountain(sb, 0.5f, 0.5f, 0.5f, 0.8f);
		//distant mountains (gray)
		drawMountain(sb, 0.25f, 0.25f, 0.7f, 0.5f);
		// distant mountains (black)
		drawMountain(sb, 0.0f, 0.0f, 0.7f, 0.3f);
	}
	...
	private void drawMountain(SpriteBatch sb, float offsetX, float offsetY, float tintColor, float parallaxSpeedX){
		...
		for(int i=0; i < mountainLength; i++){
			// mountain left
			reg = regMountainLeft;
			sb.draw(reg.getTexture(), origin.x + xRel + position.x * parallaxSpeedX, origin.y + yRel + position.y, origin.x,
